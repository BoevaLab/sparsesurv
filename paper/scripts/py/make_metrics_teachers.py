...
def main() -> int:
    ...
    for cancer in config["datasets"]:
        ...
        for model_type in ["breslow", "cox_nnet"]:
            lp = pd.read_csv(f"./results/kd/{model_type}/{cancer}/eta_teacher.csv")
            for i in range(25):
                ...
                try:
                    value.append(
                        concordance_index_ipcw(
                            ...
                            lp_split,
                            np.partition(time[train_split][event[train_split]], -3)[-3]
                            - 1e-8,
                            1e-8,
                        )[0]
                    )
    ...
    ).to_csv("./results/metrics/metrics_overall_teachers.csv", index=False)
    return 0
