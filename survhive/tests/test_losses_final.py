...
def test_breslow_loss():
        ...
        for scenario in scenarios:
                ...
                #assert np.allclose(breslow_loss*10,df_breslow[scenario], atol=1e-2)
...
def test_efron_loss():
        ...
        for scenario in scenarios:
                ...
                print(efron_loss*10, df_efron[scenario].values[0])
                #*10 due sample size divison
                assert np.allclose(efron_loss*10,df_efron[scenario])
...
def test_aft_loss_exception():
        for scenario in scenarios:
                linear_predictor, time, event = numpy_test_data_1d(scenario)
                with pytest.raises(RuntimeError) as excinfo:
                        aft_negative_likelihood(linear_predictor.reshape(dim,2), time, event)
                assert "No events detected!" in str(excinfo.value)
