...
from .compat import CVSCORERFACTORY
from .screening import StrongScreener
from .utils import estimate_group_weights, inverse_transform_survival
...
@typechecked
def _get_alpha_max_l2(
    gradient: Callable, X: np.array, groups: np.array, group_weights: np.array
):
    max_norm: float = 0.0
    for group in range(len(groups)):
        max_norm = max(
            max_norm,
            np.linalg.norm(gradient.T, X[:, groups[group]] * group_weights[group]),
        )
    return max_norm
...
def _alpha_grid_l2(
    ...
    alpha_max = (
        _get_alpha_max_l2(
            gradient=gradient, X=X, groups=groups, group_weights=group_weights
        )
        / l1_ratio
    )
    if alpha_max <= np.finfo(float).resolution:
...
def regularisation_path(
    ...
    y: np.array,
    X_test: np.array,
    model: RegularizedLinearSurvivalModel,
    group_weights=None,
    l1_ratio: float = 1.0,
    eps: float = 0.01,
    n_alphas: int = 100,
    ...
    by_group_group_weights = []
    for group in model.groups:
        by_group_group_weights.append(group_weights[group[0]])
    by_group_group_weights = np.array(by_group_group_weights)

    if alphas is None:
        if model.type == "elastic_net":
        ...
        elif model.type == "group_lasso":
            alphas = _alpha_grid_l2(
                X=X,
                y=y,
                groups=model.groups,
                gradient=gradient,
                ...
                n_alphas=n_alphas,
                groups=model.groups,
                group_weights=by_group_group_weights,
            )
    elif len(alphas) > 1:
    ...
    if type == "group_lasso":
        ...
        strong_screener = StrongScreener(
            p=X.shape[1],
            type="group_lasso",
            l1_ratio=None,
            groups=model.groups,
    ...
    for i, alpha in enumerate(alphas):
        ...
        for q in range(n_irls_iter):
            ...
            # If yes: We fit only on the strong set.
                ...
                if strong_screener.strong_set.shape[0] == 0:
                    ...
                    if model.groups is not None:
                        optimiser.__setattr__(
                            "groups", model.groups[strong_screener.strong_set]
                        )
                        optimiser.__setattr__(
                            "weights",
                            group_weights[strong_screener.strong_set],
                        )
                    optimiser.fit(
                        X=X_working_set
                        * np.sqrt(weights_scaled)
            ...
            # set and only then check potential strong set violations.
                ...
                if model.groups is not None:
                    optimiser.__setattr__(
                        "groups", model.groups[strong_screener.working_set]
                    )
                    optimiser.__setattr__(
                        "weights", group_weights[strong_screener.working_set]
                    )
                ...
                while True and q < 1:
                    ...
                    if model.groups is not None:
                        ...
                        optimiser.__setattr__(
                            "weights",
                            group_weights[strong_screener.working_set],
                        )
                    optimiser.fit(
                        X=X_working_set
                        * np.sqrt(weights_scaled)
                    ...
                    while True:
                        ...
                        if strong_screener.any_kkt_violated.shape[0] > 0:
                            ...
                            if model.groups is not None:
                                optimiser.__setattr__(
                                    "groups",
                                    model.groups[strong_screener.working_set],
                                )
                                optimiser.__setattr__(
                                    "weights",
                                    group_weights[strong_screener.working_set],
...
class CrossValidation(LinearModelCV):
    ...
    def __init__(
        ...
        if isinstance(cv, numbers.Integral):
            self.cv = StratifiedKFold(cv, shuffle=True, random_state=random_state)
    ...
    def fit(
        ...
        if model.type == "group_lasso":
            if model.has_overlaps:
                X_sorted = X_sorted[:, model.group_mapping]
            group_weights = [
                estimate_group_weights(
                    groups=model.groups,
                    model=self.group_weights,
                    l1_ratio=l1_ratio,
                )
                for l1_ratio in l1_ratios
            ]
        else:
            group_weights = [None for l1_ratio in l1_ratios]
        ...
        jobs = (
            ...
            for this_l1_ratio, this_alphas, this_group_weights in zip(
                l1_ratios, alphas, group_weights
            )
